...
while True:
    ...
    for event in pygame.event.get():
        ...
        if keys[pygame.K_a]:
            angle_y += ROTATE_SPEED #rotation
        if keys[pygame.K_d]:
            angle_y -= ROTATE_SPEED #rotation
        if keys[pygame.K_w]:
            angle_x += ROTATE_SPEED #rotation
        if keys[pygame.K_s]:
            angle_x -= ROTATE_SPEED #rotation
        if keys[pygame.K_q]:
            angle_z -= ROTATE_SPEED #rotation
        if keys[pygame.K_e]:
            angle_z += ROTATE_SPEED #rotation
        if keys[pygame.K_u]:
            save_scale = save_scale + 1
            if shape == "cube":
                inc_by_1(cube_points, save_scale) #scaling
            else:
                inc_by_1(pyramid_points, save_scale) #scaling
        if keys[pygame.K_i]:
            if shape == "cube":
                dec_by_1(cube_points, save_scale) #scaling
            else:
                dec_by_1(pyramid_points, save_scale) #scaling
        if keys[pygame.K_o]:
            save_scale = save_scale + 1
            if shape == "cube":
                inc_pos(cube_points, save_pos) #translation
            else:
                inc_pos(pyramid_points, save_pos) #translation
        if keys[pygame.K_p]:
            if shape == "cube":
                dec_pos(cube_points, save_pos) #translation
            else:
                dec_pos(pyramid_points, save_pos) #translation
